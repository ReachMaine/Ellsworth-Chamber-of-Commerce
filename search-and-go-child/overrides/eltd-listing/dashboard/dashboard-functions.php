...
if(!function_exists( 'eltd_listing_set_user_nonce' )) {
	...
	function eltd_listing_set_user_nonce() {
		...
		if(is_user_logged_in()) {
			...
			wp_localize_script('search-and-go-elated-modules', 'eltdUpdateProfile', $parameters);
		}
	}
	...
	function eltd_listing_user_whislist( $params = array()) {
		...
		if ( $users_wishlists ) {
			...
			if ( $query->post_count ) {
				...
				$html .= eltd_listing_dashboard_page_top_area(esc_html__('My Wishlist', 'eltd_listing'),esc_html__($params['subtitle_text'],'eltd_listing'));
...
if(!function_exists( 'eltd_listing_get_dropdown_tax_terms' )){
	...
	function eltd_listing_get_dropdown_tax_terms($args = array()){
		...
		$args = wp_parse_args( $args, apply_filters( 'get_terms_defaults', $defaults, $args['taxonomy']) );
